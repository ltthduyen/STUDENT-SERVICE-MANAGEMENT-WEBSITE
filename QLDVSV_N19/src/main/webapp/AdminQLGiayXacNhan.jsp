					...
					<div class="card">
						<div class="card-body">
							<table class="table table-striped" id="table1">
								<thead>
									<tr>
										<th>Mã số giấy xác nhận</th>
										<th>Tên</th>
										<th>Trạng thái</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach var="gxn" items="${listGXN}">
											<tr>
												<td><c:out value="${gxn.getMaGXN()}" /></td>
												<td><c:out value="${gxn.getTenGXN()}" /> 
												</td>
												<c:if test="${gxn.getTrangThai() == 1}">
													<td>
													<a href="GXN?action=on&gxn=<c:out value='${gxn.getMaGXN()}'/>&id=${taikhoan}&email=${email}">Bật bảo trì</a>
												</td>
												</c:if>
												<c:if test="${gxn.getTrangThai() == 0}">
													<td>
													<a href="GXN?action=off&gxn=<c:out value='${gxn.getMaGXN()}'/>&id=${taikhoan}&email=${email}">Tắt bảo trì</a>
												</td>
												</c:if>
